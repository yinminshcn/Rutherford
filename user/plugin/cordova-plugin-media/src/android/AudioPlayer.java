...
//    private static int MEDIA_ERR_NONE_SUPPORTED = 4;
    ...
    public boolean isDataURI(String file) {
        return file.matches("data.*");
    }
    ...
    private void loadAudioFile(String file) throws IllegalArgumentException, SecurityException, IllegalStateException, IOException {
        if (this.isDataURI(file)) {
            String base64FormattedString;
            String pattern = "base64,";
            int idx = file.indexOf(pattern) + pattern.length();
            base64FormattedString = file.substring(idx);
            byte data[] = Base64.decode(base64FormattedString, Base64.DEFAULT);
            this.player.setDataSource(new MediaDataSource() {
                @Override
                public long getSize() {
                    return data.length;
                }
                ...
                public int readAt(long position, byte[] buffer, int offset, int size) {
                    long xlength;
                    xlength = getSize();
                    LOG.d(LOG_TAG, "readAt: position=" + position + " offset=" + offset + " size=" + size + " length=" + xlength);
                    if (position >= xlength) return -1; // EOF
                    if (position + size > xlength) {
                        int xsize = (int)(xlength - position);
                        System.arraycopy(data, (int) position, buffer, offset, xsize);
                        return xsize;
                    } else {
