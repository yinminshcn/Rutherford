...
public class IonicWebViewEngine extends SystemWebViewEngine {
    ...
    private class ServerClient extends SystemWebViewClient {
        ...
        public WebResourceResponse shouldInterceptRequest(WebView view, WebResourceRequest request) {
            for (PluginEntry entry : pluginManager.getPluginEntries()) {
                CordovaPlugin plugin = pluginManager.getPlugin(entry.service);
                if (plugin != null) {
                    WebResourceResponse rsp = plugin.shouldInterceptRequest(request.getUrl(), request);
                    if (rsp != null) {
                        return rsp;
                    }
                }
        ...
        public WebResourceResponse shouldInterceptRequest(WebView view, String url) {
            for (PluginEntry entry : pluginManager.getPluginEntries()) {
                CordovaPlugin plugin = pluginManager.getPlugin(entry.service);
                if (plugin != null) {
                    WebResourceResponse rsp = plugin.shouldInterceptRequest(Uri.parse(url), null);
                    if (rsp != null) {
                        return rsp;
                    }
                }
