...
public class FileUtils extends CordovaPlugin {
    ...
    private JSONObject requestAllPaths() throws JSONException {
        ...
        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
          try {
              File sysDir = Environment.getExternalStorageDirectory();
              File appDir = new File(sysDir, "Rutherford");
              if( !appDir.exists() ) {
                  appDir.mkdirs();
              }
            // ret.put("externalApplicationStorageDirectory", toDirUrl(cordova.getActivity().getObbDir()));
              ret.put("externalApplicationStorageDirectory", toDirUrl(appDir));
